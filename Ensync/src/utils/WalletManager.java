...
public final class WalletManager{
    ...
    // DATABASE URL
    private final String databaseURL = "jdbc:sqlite:data/database/Ensync.db";
    ...
    private Connection connect(String url){
        ...
        if(!new File("data/database/Ensync.db").exists()){
    ...
    public boolean createWallet(String wallet, String password, String salt) throws IOException, BadNetworkException, InterruptedException{
        ...
        this.bitcoin = new Bitcoin(this.network, new File("data/temp_wallet_data"), "wallet", null);
        ...
        newWalletEntity.setWalletBytes(Files.readAllBytes(new File("data/temp_wallet_data/wallet.wallet").toPath()));
        newWalletEntity.setChainBytes(Files.readAllBytes(new File("data/temp_wallet_data/wallet.spvchain").toPath()));
    ...
    public boolean login(String walletName, String walletPassword) throws BadNetworkException, IOException {
        ...
        if(!this.wallets.isEmpty() && this.wallets.containsKey(walletName)){
            ...
            if(walletEntityObject.getWallet().equals(walletName) && walletEntityObject.getPassword().equals(hashedPassword)){
                File walletDir = new File("data/temp_wallet_data");
                if(!walletDir.exists()){
                    walletDir.mkdirs();
                }
                // Writing wallet and spv chain file bytes to disk.
                try(FileOutputStream fos = new FileOutputStream("data/temp_wallet_data/wallet.wallet")){
                    fos.write(walletEntityObject.getWalletBytes());
                }
                try(FileOutputStream fos = new FileOutputStream("data/temp_wallet_data/wallet.spvchain")){
    ...
    public void clearWalletData(boolean terminate, boolean gc, long millis){
        ...
        File[] walletDataFiles = new File("data/temp_wallet_data").listFiles();
